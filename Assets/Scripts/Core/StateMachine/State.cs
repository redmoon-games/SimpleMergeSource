...
namespace Core.StateMachine
{
    ...
    public interface IState<TEnum> where TEnum : Enum
    {
        ...
        /// </summary>
        void ChangeState(TEnum enumState);
    ...
    public abstract class AbstractState<TEnum> : IState<TEnum> where TEnum : Enum
    {
        ...
        /// </summary>
        public void ChangeState(TEnum enumState)
        {
            // Try to find the specified state.
            IState<TEnum> newState;
            if (!children.TryGetValue(enumState, out newState))
            {
                throw new ApplicationException("Tried to change to state \"" + enumState + "\", but it is not in the list of children.");
            }
        ...
        public void AddChild(IState<TEnum> newState, TEnum enumState)
        {
            ...
            {
                throw new ApplicationException("State with name \"" + enumState + "\" already exists in list of children.");
            }
        }
