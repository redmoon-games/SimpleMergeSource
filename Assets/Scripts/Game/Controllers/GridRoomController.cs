...
namespace Game.Controllers
{
    ...
    public class GridRoomController : MonoBehaviour
    {
        ...
        private readonly Dictionary<UnitController, CellController> _cells =
            new Dictionary<UnitController, CellController>();
        ...
        public void Init(RoomData data, Rect viewRect)
        {
            ...
            _signalBus.Subscribe<MergeChainEndedSignal>(
                MergeChainEndedSignal.CreateRunDelegate(_units, OnMergeChainEnded));
            ...
            _signalBus.Subscribe<UnitSacrificeSignal>(SacrificeUnit);
        }

        private void SacrificeUnit(UnitSacrificeSignal signal)
        {
            var unit = GetUnitById(signal.UnitId);
            if (unit == null) return;
        ...
        private UnitController GetUnitById(string unitID)
        {
            foreach (var unitData in _units)
            {
                if (unitData.Value.data.id == unitID)
                {
                    return unitData.Key;
                }
            }
        ...
        private void OnAnimComplete(UnitController source, UnitController destination,
            Action<CellController> onComplete)
        {
        ...
        {
            Merge(data, cell => { _signalBus.Fire(new ChainEndedSignal(new ChainEndedData(data.chain, this), this)); });
        }
        ...
        private IEnumerator IdleIncome()
        {
            while (_idleIncomeUnits.Count > 0)
            {
                ...
                yield return new WaitForSeconds(Random.Range(_incomeSettings.incomeDelayMin,
                    _incomeSettings.incomeDelayMax));
            }
        }
    }
}