...
namespace Game.UI.Popups.UnitShop
{
    ...
    public class UnitShop : Window<UnitShop>
    {
        ...
        private Dictionary<UnitShopItem, UnitShopItemParams> _itemParams =
            new Dictionary<UnitShopItem, UnitShopItemParams>();
        ...
        public void Show()
        {
            if (_windowManager.GetWindow(WindowId) == null)
            {
                ...
                _signalBus.Fire(new DownBarShowedSignal(EDownBarButtonType.Shop));
            }
        }
        ...
        public void OnCloseClick()
        {
            ...
            {
                Close();
                _signalBus.Fire(new DownBarClosedSignal(EDownBarButtonType.Shop));
            }
        }
        ...
        private void OnUnitOpened(UnitOpenedSignal signal)
        {
            ...
            {
                int maxUnlockedLevel = GetMaxUnlockedLevel(_currentShopParams.chain, _currentShopParams.startLevel,
                    _currentShopParams.maxLevel);

                foreach (var item in _itemParams)
                {
                    item.Value.state = GetUnitShopItemState(item.Value.level, maxUnlockedLevel,
                        _currentShopParams.startLevel);
        ...
        private UnitShopItemState GetUnitShopItemState(int level, int maxUnlockedLevel, int startLevel)
        {
            ...
            if (LevelNotMax(level, maxUnlockedLevel) && roomLevel > _gameSettings.unitShopOnlyGoldCount)
            {
        ...
        private bool LevelNotMax(int level, int maxUnlockedLevel)
        {
            return level >= maxUnlockedLevel -
                (_gameSettings.unitShopUnlockShift + _gameSettings.unitShopCrystalBuyShift);
        }
    }
}